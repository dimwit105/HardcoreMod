...
public class PlayerStalker
{
    ...
    {
    	if(ep.getDataWatcher().getWatchableObjectInt(DayzMod.shockDataWatcher)/20 > ((int)ep.getHealth() + (int)ep.getAbsorptionAmount()))
    	{
    		return true;
    	}
    	else
	...
	public void onEntityConstructing(EntityConstructing event)
	{
		...
		if (event.entity instanceof EntityPlayer)
		{
			...
			dw.addObject(DayzMod.shockDataWatcher, Integer.valueOf(0)); // Shock- used for KO
	        ExtendedPlayer.register((EntityPlayer) event.entity);
		}
	...
	public void playerWakeUp(PlayerWakeUpEvent event)
	{
		...
		else if(props.wellness("get", 0) > 15)
		{
			if(props.wellnessCD("get", 0) <= 0)
			{
				event.entityPlayer.heal(2);
	...
	public void onDeathEvent(LivingDeathEvent event)
	{
		...
		if(event.entity instanceof EntityPlayer)
		{
			...
			if(props.wellness("get", 0) > 10)
			{
				props.wellness("set", 10);
			}
			if(props.wellness("get", 0) > 3)
			{
				props.wellness("set", props.wellness("get", 0) - ep.worldObj.difficultySetting.getDifficultyId());
			}
			...
			if(event.source == DayzMod.zombieInfection || props.venomed("get", 0) == 4)
			{
				EntityPlayer ez = (EntityPlayer) event.entity;
				World world = event.entity.worldObj;
			...
			{
				proxy.storeEntityData(((EntityPlayer) event.entity).getCommandSenderName(), props.wellness("get", 0));
			}
			World w = event.entity.worldObj;
			NBTTagCompound worldNBT = w.getWorldInfo().getNBTTagCompound();
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event)
    {
    	...
    	//render.modelBipedMain.bipedBody.setRotationPoint(0F, 0F, 0F);
    	//event.renderer.modelBipedMain.bipedBody.rotateAngleX = 9.15F;
    	//event.renderer.modelBipedMain.bipedBody.rotateAngleY = 9.15F;
    	//event.renderer.modelBipedMain.bipedBody.rotateAngleZ = 9.15F;
    	//render.modelBipedMain.bipedBody.
    	//event.renderer.modelBipedMain.bipedBody.offsetX = 0.15F;
	...
	{	
		if(event.entity instanceof EntityPlayer)
		{
			...
			ExtendedPlayer props = ExtendedPlayer.get((EntityPlayer) event.entity);
			DataWatcher dw = event.entity.getDataWatcher();
			float finalDamage = ArmorProperties.ApplyArmor(ep, ep.inventory.armorInventory, event.source, event.ammount);
			finalDamage = UtilMisc.applyPotionDamageCalculations(event.source, finalDamage, ep);
			...
			if(event.ammount > 0 && rand.nextInt(4) == 0 && !ep.capabilities.isCreativeMode && ep.getAbsorptionAmount() <= 0)
			{
				if(this.isKO(ep))
				{
					...
					{
						int diff = (int) ((int)dw.getWatchableObjectInt(DayzMod.shockDataWatcher) + 15 * UtilMisc.getDifficultyMod(event.entity.worldObj));
						dw.updateObject(DayzMod.shockDataWatcher, diff);
					}
				}
				else
				{
					int diff = (int) ((int)dw.getWatchableObjectInt(DayzMod.shockDataWatcher) + 20 * UtilMisc.getDifficultyMod(event.entity.worldObj));
					//dw.updateObject(DayzMod.shockDataWatcher, diff);
				}
			}
			if(event.source == DamageSource.fall)
			{
				dw.updateObject(DayzMod.shockDataWatcher, dw.getWatchableObjectInt(DayzMod.shockDataWatcher) + (int)event.ammount*10);
				if(rand.nextInt(3) == 0)
				{
					int add = 0;
					add = (int) ((int) (event.ammount * 1800)*UtilMisc.getDifficultyMod(event.entity.worldObj));
					if(add != 0)
					{
			...
			if(event.source.getEntity() instanceof EntityZombie)
			{
				...
				if(item != null)
				{
					if(item.getItem() == DayzMod.envenomedSword && props.venomed("get", 0) == 0)
					{
						int Time = 7200 + rand.nextInt(4800);
						props.venomTime("set", Time);
						props.venomed("set", 1);
					}
			...
			if(event.source.getEntity() != null && Math.floor(finalDamage) >= 1F)
			{
				double fd = Math.floor(finalDamage);
				//System.out.println("Damage: " + finalDamage + ", " + fd);
				int bleed = (int) (Math.min(rand.nextInt((int)fd) + 3, fd));
				//int bleed = 0;
				if(bleed > props.bleed("get", 0) && finalDamage > (ep.getEntityAttribute(SharedMonsterAttributes.maxHealth).getAttributeValue()*0.33F) / UtilMisc.getDifficultyMod(event.entity.worldObj))
				{
			...
			if(event.source.getEntity() instanceof EntityCaveSpider && props.venomed("get", 0) == 0)
			{
				props.venomTime("set", 7200 + rand.nextInt(4800));
				props.venomed("set", 1);
			}
			if(event.source.getEntity() instanceof EntitySpider && props.venomed("get", 0) == 0 && rand.nextInt(12) == 0 && !(event.source.getEntity() instanceof EntityPoisonSpider))
			{
				props.venomTime("set", 24000 + rand.nextInt(12000));
				props.venomed("set", 3);
			}
			if(event.source.getEntity() instanceof EntityZombie && props.venomed("get", 0) == 0 && rand.nextInt(DayzMod.corruptChance) == 0)
			{
	...
	public void onHeal(LivingHealEvent event)
	{
		if(event.entity instanceof EntityPlayer)
		{
			...
			{
				float percent = Math.max((float)props.bloodLevel("get", 0) / 2000, 0.1F);
				event.amount = event.amount * percent;
				System.out.println("Heal percent: " + percent);
			}
			if(props.bleed("get", 0) > 0)
			{
				
				event.amount = (float) (event.amount / Math.sqrt((props.bleed("get", 0))));
				System.out.println(ep.getDisplayName() + " tried to heal while bleeding, amount divided by: " + Math.sqrt((props.bleed("get", 0))) + " which ends with; " + event.amount);
			}
			if(props.badBlood("get", false))
			{
				event.setCanceled(true);
			}
		}
	...
	public void onUpdate2Event(LivingUpdateEvent event)
	{
		if(event.entity instanceof EntityPlayer)
		{
			...
			DataWatcher dw = event.entity.getDataWatcher();
			ExtendedPlayer props = ExtendedPlayer.get((EntityPlayer) event.entity);
			if(((props.venomed("get", 0) == 1 && props.venomTime("get", 0) <= 1200)))
			{
	...
	//TODO make tranq arrows work
	@SubscribeEvent
	public void onBowUse(ArrowLooseEvent event)
	{
		int j = event.charge;
		EntityPlayer ep = event.entityPlayer;
		ep.inventory.hasItem(null);
	}
	@SubscribeEvent
	...
	{	
		...
		if(!(dw.getWatchableObjectInt(DayzMod.shockDataWatcher)/20 > player.getHealth()))
		{
			if(mc.gameSettings.showDebugInfo)
			{
				...
				event.right.add("Wellness: " + props.wellness("get", 0));
				event.right.add("Blood level: " + props.bloodLevel("get", 0) + " Healing percent: " + Math.max((float)props.bloodLevel("get", 0) / 2000, 0.1F));
			...
			event.left.add("Shock: " + dw.getWatchableObjectInt(DayzMod.shockDataWatcher) + " ("+dw.getWatchableObjectInt(DayzMod.shockDataWatcher)/20 +")");
			if(props.bleed("get", 0) > 0)
			{
			...
			*/
			...
			{
				event.left.add("\u00A7dTime left: \u00A7d" + (props.venomTime("get", 0)/20) + "s State:" + props.venomed("get", 0));
			}
			/*
        ...
        if(mc.thePlayer.getDataWatcher().getWatchableObjectInt(DayzMod.shockDataWatcher)/20 > (mc.thePlayer.getHealth() + mc.thePlayer.getAbsorptionAmount()) && !mc.thePlayer.capabilities.isCreativeMode && !mc.thePlayer.isDead)
        {	
        	String stringToDisplay = "Shock: " + mc.thePlayer.getDataWatcher().getWatchableObjectInt(DayzMod.shockDataWatcher)/20;
